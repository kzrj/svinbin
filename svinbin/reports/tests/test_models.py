...
class OperationDataMegalistTest(TransactionTestCase):
    def setUp(self):
        locations_testing.create_workshops_sections_and_cells()
        sows_testings.create_statuses()
        sows_events_testings.create_types()
    ...
    def test_gen_megadict_two_operations_filter_sow(self):
        ...
        sow1 = sows_testings.create_sow_with_semination_usound(location=self.loc_ws1,
            week=22)
        sow2 = sows_testings.create_sow_with_semination_usound(location=self.loc_ws1,
            week=22)

        date_10d_ago = timezone.now() - timedelta(10)
        sow3 = sows_testings.create_sow_with_semination_usound(location=self.loc_ws1,
            week=22, date=date_10d_ago)
        sow4 = sows_testings.create_sow_with_semination_usound(
            location=self.loc_ws3_cells[0],
            week=22, date=date_10d_ago)

        farrow = SowFarrow.objects.create_sow_farrow(sow=sow4, alive_quantity=10,
         date=date_10d_ago)
        piglets = farrow.piglets_group

        CullingPiglets.objects.create_culling_piglets(piglets_group=piglets,
         culling_type='padej', date=date_10d_ago)
...
class ReportDateWSReportTest(TransactionTestCase):
    ...
    def test_count_sow_cullings_ws3_today(self):
        sow1 = sows_testings.create_sow_and_put_in_workshop_one()
        sow2 = sows_testings.create_sow_and_put_in_workshop_one()
        sow3 = sows_testings.create_sow_and_put_in_workshop_one()
        sow4 = sows_testings.create_sow_and_put_in_workshop_one()
        ...
        with freeze_time("2020-06-10"):
            CullingSow.objects.create_culling(sow=sow1, culling_type='padej')
            CullingSow.objects.create_culling(sow=sow2, culling_type='padej')

        # day1_rd = ReportDate.objects.get(date=date(2020, 6, 10))
        # print(CullingSow.objects.all().first().date)
        # ws_locs = Location.objects.all().get_workshop_location_by_number(workshop_number=3)
        # print(CullingSow.objects.filter(location__in=ws_locs, date__date=day1_rd.date))

        
        # self.assertEqual(day1_rd.count_sow_cullings_in_ws3_today['suporos'], 2)
        # self.assertEqual(day1_rd.count_sow_cullings_in_ws3_today['podsos'], 0)

        # day2_rd = ReportDate.objects.get(date=date(2020, 5, 14))
        # self.assertEqual(day2_rd.count_sow_cullings_in_ws3_today['suporos'], 0)
        # self.assertEqual(day2_rd.count_sow_cullings_in_ws3_today['podsos'], 0)