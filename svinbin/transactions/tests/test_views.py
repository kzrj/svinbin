...
#         self.test_view = WorkShopSowTransactionViewSet()
...
#         self.test_view.move_to(sow, Section.objects.get(workshop__number=1, number=1))

#         sow.refresh_from_db()
#         self.assertEqual(sow.location.section.number, 1)

#     def test_move_to_workshop(self):
#         sow = sows_testing.create_sow_and_put_in_workshop_two(2, '2')
#         self.test_view.move_to(sow, WorkShop.objects.get(workshop__number=1))
...
#     def test_move_to_workshop_two(self):
#         sow = sows_testing.create_sow_and_put_in_workshop_one(1, '200')
#         self.assertEqual(sow.location.sowSingleCell.number, '200')

#         response = self.client.post('/api/workshopone/sowtransactions/%s/move_to_workshop_two/'\
#          % sow.pk)
...
#         self.assertEqual(sow.location.workshop.number, 1)
    ...
    #     response = self.client.post('/api/workshopone/sowtransactions/%s/move_to_workshop_one/' % sow.pk,
    #      {'sow': sow.pk})
