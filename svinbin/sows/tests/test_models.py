...
class SowModelManagerTest(TransactionTestCase):
    ...
    def test_queryset_serializer(self):
        ...
        with self.assertNumQueries(3):
            data = Sow.objects.all().select_related('location', 'status', 'tour') \
                .prefetch_related(
                    Prefetch(
                        'semination_set__tour',
                        queryset=Semination.objects.filter(tour=F('tour')),
                        to_attr='current_tour_seminations'
                    )
                ) 
                # .prefetch_related(
                #     Prefetch(
                #         'ultrasound_set',
                #         queryset=Ultrasound.objects.filter(tour=F('tour'))
                #     )
                # )
